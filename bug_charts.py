#pylint: disable=line-too-long, unspecified-encoding, unused-variable, invalid-name
'''Charting script'''
import os
...
def count_by_priority(bug_df, png_dir, html_dir):
    ...
    bug_df['Priority'] = pd.Categorical(bug_df["Priority"], ['Blocker', 'Major', 'Minor', 'Trivial', 'Non Production'])
    fig, ax = plt.subplots(figsize=(12,8))
    palette = {'Customer Action':'tab:red','In Progress':'tab:blue', 'Open':'tab:orange', 'Resolved':'tab:green', 'in Discussion':'tab:grey', 'REOPEN':'tab:orange', 'HOLD':'tab:grey'}
    sns.set_context('notebook')
